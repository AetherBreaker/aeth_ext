...
class TestConfigured:
  ...
  def test_posts_the_title_and_message_to_the_pushover_api(self, monkeypatch: pytest.MonkeyPatch):
  ...
  def test_emergency_priority_adds_retry_and_expire_params(self, monkeypatch: pytest.MonkeyPatch):
    requests: list[Request] = []
  ...
  def test_non_200_status_is_logged_not_raised(self, monkeypatch: pytest.MonkeyPatch, caplog: pytest.LogCaptureFixture):
  ...
  def test_url_error_is_caught_and_logged_not_propagated(self, monkeypatch: pytest.MonkeyPatch, caplog: pytest.LogCaptureFixture):
  ...
  def test_no_attachment_fields_when_image_is_none(self, monkeypatch: pytest.MonkeyPatch):
    requests: list[Request] = []
  ...
  def test_image_is_base64_encoded_and_attached(self, monkeypatch: pytest.MonkeyPatch):
    requests: list[Request] = []
  ...
  def test_message_under_the_length_limit_is_sent_unchanged(self, monkeypatch: pytest.MonkeyPatch):
    requests: list[Request] = []
  ...
  def test_message_over_the_length_limit_is_truncated_with_a_note(self, monkeypatch: pytest.MonkeyPatch):
    requests: list[Request] = []
    monkeypatch.setattr(sap_mod, "urlopen", lambda request, timeout: (requests.append(request), _FakeResponse(200))[1])
