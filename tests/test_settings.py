...
class TestFieldDefaults:
  def test_alerts_email_pwd_is_required(self) -> None:
    with pytest.raises(Exception):  # noqa: B017 - pydantic ValidationError
      BaseSettings(_env_file=None)  # pyright: ignore[reportCallIssue]

  def test_default_alerts_recipients_is_a_frozenset(self) -> None:
    settings = _make_settings()

    assert isinstance(settings.alerts_recipients, frozenset)

  def test_pushover_settings_default_to_none(self) -> None:
    settings = _make_settings()

    assert settings.alerts_pushover_token is None
    assert settings.alerts_pushover_user_key is None

  def test_healthcheck_ping_url_defaults_to_none(self) -> None:
    settings = _make_settings()

    assert settings.alerts_healthcheck_ping_url is None

  def test_healthcheck_pingkey_defaults_to_none(self) -> None:
    settings = _make_settings()

    assert settings.alerts_healthcheck_pingkey is None

  def test_log_loc_folder_defaults_under_persisted_dir_loc(self) -> None:
    settings = _make_settings()

    assert settings.log_loc_folder == settings.persisted_dir_loc / "logs"

  def test_log_loc_folder_tracks_an_overridden_persisted_dir_loc(self, tmp_path: Path) -> None:
  ...
  def test_explicit_log_loc_folder_overrides_the_derived_default(self, tmp_path: Path) -> None:
...
class TestEnvVarOverride:
  def test_env_var_overrides_a_default(self, monkeypatch: pytest.MonkeyPatch) -> None:
  ...
  def test_kwarg_takes_precedence_over_env_var(self, monkeypatch: pytest.MonkeyPatch) -> None:
...
class TestAddressLikeCoercion:
  @given(st.emails())
  def test_plain_string_address_is_accepted_as_is(self, email: str) -> None:
  ...
  def test_an_actual_address_object_is_accepted_as_is(self) -> None:
  ...
  def test_recipients_frozenset_round_trips(self, recipients: frozenset[str]) -> None:
...
class TestGetSettings:
  def test_get_settings_returns_a_base_settings_instance(self, monkeypatch: pytest.MonkeyPatch) -> None:
