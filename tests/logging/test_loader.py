...
class TestLoadPackagedFragment:
  ...
  def test_all_fragments_load(self, name: str) -> None:
...
class TestAssembleDefaultConfig:
  ...
  def test_standalone_combos_validate(self, names: tuple[str, ...]) -> None:
    config = loader.assemble_default_config(*names)
    LoggingConfigModel.model_validate(config)

  def test_console_fragment_appends_to_root_handlers(self) -> None:
  ...
  def test_async_queue_replaces_root_handlers_with_runtime_ref(self) -> None:
  ...
  def test_socket_client_deep_merges_into_main(self) -> None:
  ...
  def test_file_fragments_share_handler_names(self) -> None:
  ...
  def test_no_merge_markers_survive(self) -> None:
    config = loader.assemble_default_config("main_base", "file_daily", "console_rich", "async_queue")
...
class TestFindOverrideConfig:
  def test_returns_none_when_nothing_found(self, _no_override: None) -> None:
    assert loader.find_override_config() is None

  def test_settings_loc_as_file(self, _no_override: None, monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
  ...
  def test_settings_loc_as_directory(self, _no_override: None, monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
  ...
  def test_entrypoint_dir_fallback(self, _no_override: None, monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
  ...
  def test_cwd_fallback(self, _no_override: None, monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
  ...
  def test_entrypoint_wins_over_cwd(self, _no_override: None, monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    main_dir = tmp_path / "app"
    main_dir.mkdir()
...
class TestLoadEffectiveConfig:
  def test_no_override_returns_assembled_default(self, _no_override: None) -> None:
  ...
  def test_replace_mode_swaps_config_wholesale(self, _no_override: None, tmp_path: Path) -> None:
  ...
  def test_replace_mode_strips_markers(self, _no_override: None, tmp_path: Path) -> None:
    override = tmp_path / "override.toml"
    override.write_text('version = 1\n\n[root]\n__merge__ = "deep"\nlevel = "WARNING"\n', encoding="utf-8")
    config = loader.load_effective_config(("main_base",), override_path=override)
    assert config["root"] == {"level": "WARNING"}

  def test_merge_mode_merges_onto_default(self, _no_override: None, tmp_path: Path) -> None:
    override = tmp_path / "override.toml"
  ...
  def test_override_filename_selects_alternate_file(self, _no_override: None, monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    work_dir = tmp_path / "work"
    work_dir.mkdir()
    (work_dir / loader.DEFAULT_OVERRIDE_FILENAME).write_text('version = 1\n\n[root]\nlevel = "ERROR"\n', encoding="utf-8")
...
class TestPreResolve:
  def test_resolves_client_side_prefixes(self, monkeypatch: pytest.MonkeyPatch) -> None:
  ...
  def test_server_side_prefixes_left_untouched(self) -> None:
    config = {
      "version": 1,
  ...
  def test_unresolvable_value_raises(self) -> None:
