...
class TestIsWebViewerEntrypoint:
  def test_true_when_main_file_is_web_viewers_own_main(self, monkeypatch: pytest.MonkeyPatch):
  ...
  def test_false_for_a_different_entrypoint_file(self, monkeypatch: pytest.MonkeyPatch, tmp_path: Path):
  ...
  def test_false_when_main_has_no_file_attribute(self, monkeypatch: pytest.MonkeyPatch):
...
class TestPatchTextualLoggerGuard:
  def test_does_nothing_when_not_the_web_viewer_entrypoint(self, monkeypatch: pytest.MonkeyPatch):
  ...
  def test_patches_textual_logger_call_when_it_is_the_entrypoint(self, monkeypatch: pytest.MonkeyPatch):
    real_main_file = Path(wv_patches.__file__).parent / "__main__.py"
    monkeypatch.setitem(sys.modules, "__main__", SimpleNamespace(__file__=str(real_main_file)))
  ...
  def test_routes_textual_log_calls_through_stdlib_logging(self, monkeypatch: pytest.MonkeyPatch, caplog: pytest.LogCaptureFixture):
    real_main_file = Path(wv_patches.__file__).parent / "__main__.py"
    monkeypatch.setitem(sys.modules, "__main__", SimpleNamespace(__file__=str(real_main_file)))
